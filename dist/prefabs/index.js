...
function UserChat(_a) {
  ...
  return /* @__PURE__ */ React7.createElement(
    ...
    /* @__PURE__ */ React7.createElement("span", { className: "lk-meta-data" }, /* @__PURE__ */ React7.createElement("strong", { className: "lk-participant-name" }, (_d = (_b2 = entry.from) == null ? void 0 : _b2.name) != null ? _d : (_c = entry.from) == null ? void 0 : _c.identity)),
...
function useChat(options) {
  ...
  React8.useEffect(() => {
    const setupChatReturn = (0, import_components_core2.setupChat)(room, options);
...
function Chat(_a) {
  ...
  return /* @__PURE__ */ React8.createElement("div", __spreadProps(__spreadValues({}, props), { className: "lk-chat" }), /* @__PURE__ */ React8.createElement("ul", { className: "tl-list lk-chat-messages", ref: ulRef }, props.children ? chatMessages.map(
...
function ChatEntry(_a) {
  ...
  return /* @__PURE__ */ React9.createElement(
    ...
    (!hideTimestamp || !hideName) && /* @__PURE__ */ React9.createElement("span", { className: "lk-meta-data" }, !hideName && /* @__PURE__ */ React9.createElement("strong", { className: "lk-participant-name" }, (_d = (_b2 = entry.from) == null ? void 0 : _b2.name) != null ? _d : (_c = entry.from) == null ? void 0 : _c.identity), !hideTimestamp && /* @__PURE__ */ React9.createElement("span", { className: "lk-timestamp" }, time.toLocaleTimeString(locale, { timeStyle: "short" }))),
    /* @__PURE__ */ React9.createElement("span", { className: "lk-message-body" }, formattedMessage)
  );
}
function formatChatMessageLinks(message) {
  return (0, import_components_core3.tokenize)(message, (0, import_components_core3.createDefaultGrammar)()).map((tok, i) => {
    if (typeof tok === `string`) {
      return tok;
    } else {
      const content = tok.content.toString();
      const href = tok.type === `url` ? /^http(s?):\/\//.test(content) ? content : `https://${content}` : `mailto:${content}`;
      return /* @__PURE__ */ React9.createElement("a", { className: "lk-chat-link", key: i, href, target: "_blank", rel: "noreferrer" }, content);
    }
  });
}
...
var useSize = (target) => {
  const [size, setSize] = React10.useState({ width: 0, height: 0 });
  React10.useLayoutEffect(() => {
    if (target.current) {
  ...
  const resizeCallback = React10.useCallback(
...
function useMediaQuery(query) {
  ...
  const [matches, setMatches] = React11.useState(getMatches(query));
...
function useConnectionState(room) {
  ...
  const observable = React13.useMemo(() => (0, import_components_core5.connectionStateObserver)(r), [r]);
...
function useFocusToggle({ trackRef, trackSource, participant, props }) {
  ...
  const layoutContext = useMaybeLayoutContext();
  const { className } = React15.useMemo(() => (0, import_components_core7.setupFocusToggle)(), []);
...
function useIsSpeaking(participant) {
  const p = useEnsureParticipant(participant);
  const observable = React18.useMemo(() => (0, import_components_core10.createIsSpeakingObserver)(p), [p]);
...
function useLocalParticipantPermissions() {
  const room = useRoomContext();
  const permissionObserver = React20.useMemo(
...
function useMediaDeviceSelect({
  ...
  const [currentDeviceId, setCurrentDeviceId] = React21.useState("");
  const { className, activeDeviceObservable, setActiveMediaDevice } = React21.useMemo(
...
function useMediaDevices({ kind }) {
  const deviceObserver = React22.useMemo(() => (0, import_components_core14.createMediaDeviceObserver)(kind), [kind]);
  const devices = useObservableState(deviceObserver, []);
  return devices;
}
...
function useMediaTrackBySourceOrName(observerOptions, options = {}) {
  var _a;
  const [publication, setPublication] = React23.useState((0, import_components_core16.getTrackByIdentifier)(observerOptions));
...
function useParticipantTile({
  ...
  const p = useEnsureParticipant(participant);
  const trackReference = React26.useMemo(() => {
...
function useStartAudio({ room, props }) {
  ...
  const { className, roomAudioPlaybackAllowedObservable, handleStartAudioPlayback } = React29.useMemo(
...
function useSwipe(element, options = {}) {
  var _a;
  const touchStart = React30.useRef(null);
  const touchEnd = React30.useRef(null);
  ...
  const onTouchEnd = React30.useCallback(() => {
...
function useChatToggle({ props }) {
  ...
  const { className } = React31.useMemo(() => (0, import_components_core21.setupChatToggle)(), []);
  const mergedProps = React31.useMemo(() => {
...
function useTrackMutedIndicator(trackRefOrSource, options = {}) {
  ...
  const { className, mediaMutedObserver } = React32.useMemo(
...
function useTrackToggle(_a) {
  ...
  const { toggle, className, pendingObserver, enabledObserver } = React33.useMemo(
  ...
  const newProps = React33.useMemo(() => mergeProps(rest, { className }), [rest, className]);
  const clickHandler = React33.useCallback(
    (evt) => {
      var _a3;
      toggle();
...
function useTracks(sources = [
  ...
  const room = useEnsureRoom(options.room);
  const [trackReferences, setTrackReferences] = React34.useState([]);
  const [participants, setParticipants] = React34.useState([]);
  const sources_ = React34.useMemo(() => {
  ...
  const maybeTrackReferences = React34.useMemo(() => {
...
function MediaDeviceSelect(_a) {
  ...
  return /* @__PURE__ */ React35.createElement("ul", __spreadValues({}, mergedProps), devices.map((device, index) => /* @__PURE__ */ React35.createElement(
    "li",
    {
      key: device.deviceId,
      id: device.deviceId,
    ...
    /* @__PURE__ */ React35.createElement("button", { className: "lk-button", onClick: () => handleActiveDeviceChange(device.deviceId) }, device.label)
  )));
}
...
function MediaDeviceMenu(_a) {
  ...
  const [devices, setDevices] = React36.useState([]);
  const [updateRequired, setUpdateRequired] = React36.useState(true);
  ...
  const button = React36.useRef(null);
  const tooltip = React36.useRef(null);
  ...
  const handleClickOutside = React36.useCallback(
    (event) => {
      if (!tooltip.current) {
        return;
      }
  ...
  return /* @__PURE__ */ React36.createElement(React36.Fragment, null, /* @__PURE__ */ React36.createElement(
    ...
    props.children
  ), !props.disabled && /* @__PURE__ */ React36.createElement(
    "div",
    {
    ...
    kind ? /* @__PURE__ */ React36.createElement(
      MediaDeviceSelect,
      {
        initialSelection,
    ...
    ) : /* @__PURE__ */ React36.createElement(React36.Fragment, null, /* @__PURE__ */ React36.createElement("div", { className: "lk-device-menu-heading" }, "Audio inputs"), /* @__PURE__ */ React36.createElement(
      MediaDeviceSelect,
      {
        kind: "audioinput",
    ...
    ), /* @__PURE__ */ React36.createElement("div", { className: "lk-device-menu-heading" }, "Video inputs"), /* @__PURE__ */ React36.createElement(
      MediaDeviceSelect,
      {
        kind: "videoinput",
...
var SvgChatIcon = (props) => /* @__PURE__ */ React37.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 18, fill: "none" }, props), /* @__PURE__ */ React37.createElement(
  "path",
  {
    fill: "currentColor",
    fillRule: "evenodd",
...
var SvgChevron = (props) => /* @__PURE__ */ React38.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React38.createElement(
  "path",
  {
    fill: "currentcolor",
    fillRule: "evenodd",
...
var SvgFocusToggleIcon = (props) => /* @__PURE__ */ React39.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React39.createElement("g", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }, /* @__PURE__ */ React39.createElement("path", { d: "M10 1.75h4.25m0 0V6m0-4.25L9 7M6 14.25H1.75m0 0V10m0 4.25L7 9" })));
var FocusToggleIcon_default = SvgFocusToggleIcon;

// src/assets/icons/LeaveIcon.tsx
var React40 = __toESM(require("react"));
var SvgLeaveIcon = (props) => /* @__PURE__ */ React40.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React40.createElement(
  "path",
  {
    fill: "currentColor",
    fillRule: "evenodd",
...
var SvgLockLockedIcon = (props) => /* @__PURE__ */ React41.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React41.createElement(
  "path",
  {
    fill: "currentcolor",
    fillRule: "evenodd",
...
var SvgQualityExcellentIcon = (props) => /* @__PURE__ */ React42.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "currentcolor" }, props), /* @__PURE__ */ React42.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-6a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React42.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-6a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }));
var QualityExcellentIcon_default = SvgQualityExcellentIcon;

// src/assets/icons/QualityGoodIcon.tsx
var React43 = __toESM(require("react"));
var SvgQualityGoodIcon = (props) => /* @__PURE__ */ React43.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "currentcolor" }, props), /* @__PURE__ */ React43.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React43.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React43.createElement("g", { opacity: 0.25 }, /* @__PURE__ */ React43.createElement("path", { d: "M12 .5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React43.createElement("path", { d: "M12 .5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" })));
var QualityGoodIcon_default = SvgQualityGoodIcon;

// src/assets/icons/QualityPoorIcon.tsx
var React44 = __toESM(require("react"));
var SvgQualityPoorIcon = (props) => /* @__PURE__ */ React44.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "currentcolor" }, props), /* @__PURE__ */ React44.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React44.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React44.createElement("g", { opacity: 0.25 }, /* @__PURE__ */ React44.createElement("path", { d: "M6 6.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React44.createElement("path", { d: "M6 6.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5zm6-6a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" }), /* @__PURE__ */ React44.createElement("path", { d: "M12 .5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5z" })));
var QualityPoorIcon_default = SvgQualityPoorIcon;

// src/assets/icons/QualityUnknownIcon.tsx
var React45 = __toESM(require("react"));
var SvgQualityUnknownIcon = (props) => /* @__PURE__ */ React45.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "currentColor" }, props), /* @__PURE__ */ React45.createElement("g", { opacity: 0.25 }, /* @__PURE__ */ React45.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-4Zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-9Zm6-6a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5V.5Z" }), /* @__PURE__ */ React45.createElement("path", { d: "M0 11.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-4Zm6-5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-9Zm6-6a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5V.5Z" })));
var QualityUnknownIcon_default = SvgQualityUnknownIcon;

// src/assets/icons/ScreenShareIcon.tsx
var React46 = __toESM(require("react"));
var SvgScreenShareIcon = (props) => /* @__PURE__ */ React46.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 20, height: 16, fill: "none" }, props), /* @__PURE__ */ React46.createElement(
  "path",
  {
    fill: "currentColor",
    fillRule: "evenodd",
...
var SvgScreenShareStopIcon = (props) => /* @__PURE__ */ React47.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 20, height: 16, fill: "none" }, props), /* @__PURE__ */ React47.createElement("g", { fill: "currentColor" }, /* @__PURE__ */ React47.createElement("path", { d: "M7.28 4.22a.75.75 0 0 0-1.06 1.06L8.94 8l-2.72 2.72a.75.75 0 1 0 1.06 1.06L10 9.06l2.72 2.72a.75.75 0 1 0 1.06-1.06L11.06 8l2.72-2.72a.75.75 0 0 0-1.06-1.06L10 6.94z" }), /* @__PURE__ */ React47.createElement(
  "path",
  {
    fillRule: "evenodd",
...
var SvgSpinnerIcon = (props) => /* @__PURE__ */ React48.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React48.createElement(
  "path",
  {
    fill: "currentColor",
    fillRule: "evenodd",
...
var SvgUnfocusToggleIcon = (props) => /* @__PURE__ */ React49.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 16, height: 16, fill: "none" }, props), /* @__PURE__ */ React49.createElement("g", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }, /* @__PURE__ */ React49.createElement("path", { d: "M13.25 7H9m0 0V2.75M9 7l5.25-5.25M2.75 9H7m0 0v4.25M7 9l-5.25 5.25" })));
var UnfocusToggleIcon_default = SvgUnfocusToggleIcon;

// src/assets/icons/tl/CameraDisabledIcon.tsx
var React50 = __toESM(require("react"));
var SvgCameraDisabledIcon = (props) => /* @__PURE__ */ React50.createElement(
  "svg",
  __spreadValues({
    width: 18,
    height: 18,
...
var SvgCameraIcon = (props) => /* @__PURE__ */ React51.createElement(
  "svg",
  __spreadValues({
    width: 18,
    height: 18,
...
var SvgMicDisabledIcon = (props) => /* @__PURE__ */ React52.createElement(
  "svg",
  __spreadValues({
    width: 18,
    height: 18,
...
var SvgMicIcon = (props) => /* @__PURE__ */ React53.createElement(
  "svg",
  __spreadValues({
    width: 18,
    height: 18,
...
function getSourceIcon(source, enabled) {
  switch (source) {
    case import_livekit_client6.Track.Source.Microphone:
      return enabled ? /* @__PURE__ */ React54.createElement(MicIcon_default, null) : /* @__PURE__ */ React54.createElement(MicDisabledIcon_default, null);
    case import_livekit_client6.Track.Source.Camera:
      return enabled ? /* @__PURE__ */ React54.createElement(CameraIcon_default, null) : /* @__PURE__ */ React54.createElement(CameraDisabledIcon_default, null);
    case import_livekit_client6.Track.Source.ScreenShare:
      return enabled ? /* @__PURE__ */ React54.createElement(ScreenShareStopIcon_default, null) : /* @__PURE__ */ React54.createElement(ScreenShareIcon_default, null);
    default:
      return void 0;
  }
}
function getConnectionQualityIcon(quality) {
  switch (quality) {
    case import_livekit_client6.ConnectionQuality.Excellent:
      return /* @__PURE__ */ React54.createElement(QualityExcellentIcon_default, null);
    case import_livekit_client6.ConnectionQuality.Good:
      return /* @__PURE__ */ React54.createElement(QualityGoodIcon_default, null);
    case import_livekit_client6.ConnectionQuality.Poor:
      return /* @__PURE__ */ React54.createElement(QualityPoorIcon_default, null);
    default:
      return /* @__PURE__ */ React54.createElement(QualityUnknownIcon_default, null);
  }
}
...
function TrackToggle(_a) {
  ...
  return /* @__PURE__ */ React55.createElement("button", __spreadValues({}, buttonProps), (showIcon != null ? showIcon : true) && getSourceIcon(props.source, enabled), props.children);
}
...
function usePreviewTracks(options, onError) {
  const [tracks, setTracks] = React57.useState();
  React57.useEffect(() => {
...
function usePreviewDevice(enabled, deviceId, kind) {
  const [deviceError, setDeviceError] = React57.useState(null);
  const [isCreatingTrack, setIsCreatingTrack] = React57.useState(false);
  const devices = useMediaDevices({ kind });
  const [selectedDevice, setSelectedDevice] = React57.useState(
    void 0
  );
  const [localTrack, setLocalTrack] = React57.useState();
  const [localDeviceId, setLocalDeviceId] = React57.useState(deviceId);
  React57.useEffect(() => {
  ...
  const prevDeviceId = React57.useRef(localDeviceId);
  React57.useEffect(() => {
...
function PreJoin(_a) {
  ...
  const [userChoices, setUserChoices] = React57.useState(DEFAULT_USER_CHOICES);
  const [username, setUsername] = React57.useState(
    (_a2 = defaults.username) != null ? _a2 : DEFAULT_USER_CHOICES.username
  );
  const [videoEnabled, setVideoEnabled] = React57.useState(
  ...
  const [videoDeviceId, setVideoDeviceId] = React57.useState(initialVideoDeviceId);
  const initialAudioDeviceId = (_d = defaults.audioDeviceId) != null ? _d : DEFAULT_USER_CHOICES.audioDeviceId;
  const [audioEnabled, setAudioEnabled] = React57.useState(
    (_e = defaults.audioEnabled) != null ? _e : DEFAULT_USER_CHOICES.audioEnabled
  );
  const [audioDeviceId, setAudioDeviceId] = React57.useState(initialAudioDeviceId);
  const [e2ee, setE2ee] = React57.useState((_f = defaults.e2ee) != null ? _f : DEFAULT_USER_CHOICES.e2ee);
  const [sharedPassphrase, setSharedPassphrase] = React57.useState(
  ...
  const videoEl = React57.useRef(null);
  const videoTrack = React57.useMemo(
    () => tracks == null ? void 0 : tracks.filter((track) => track.kind === import_livekit_client7.Track.Kind.Video)[0],
    [tracks]
  );
  const facingMode = React57.useMemo(() => {
    if (videoTrack) {
  ...
  const audioTrack = React57.useMemo(
  ...
  const [isValid, setIsValid] = React57.useState();
  const handleValidation = React57.useCallback(
    (values) => {
  ...
  return /* @__PURE__ */ React57.createElement("div", __spreadValues({ className: "lk-prejoin" }, htmlProps), /* @__PURE__ */ React57.createElement("div", { className: "lk-video-container" }, videoTrack && /* @__PURE__ */ React57.createElement("video", { ref: videoEl, width: "1280", height: "720", "data-lk-facing-mode": facingMode }), (!videoTrack || !videoEnabled) && /* @__PURE__ */ React57.createElement("div", { className: "lk-camera-off-note" }, /* @__PURE__ */ React57.createElement(ParticipantPlaceholder_default, null))), /* @__PURE__ */ React57.createElement("div", { className: "lk-button-group-container" }, /* @__PURE__ */ React57.createElement("div", { className: "lk-button-group audio" }, /* @__PURE__ */ React57.createElement(
    TrackToggle,
    {
    ...
    micLabel
  ), /* @__PURE__ */ React57.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React57.createElement(
    MediaDeviceMenu,
    {
  ...
  ))), /* @__PURE__ */ React57.createElement("div", { className: "lk-button-group video" }, /* @__PURE__ */ React57.createElement(
    TrackToggle,
    {
  ...
  )))), /* @__PURE__ */ React57.createElement("form", { className: "lk-username-container" }, /* @__PURE__ */ React57.createElement(
    "input",
    {
  ...
  ), showE2EEOptions && /* @__PURE__ */ React57.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "1rem" } }, /* @__PURE__ */ React57.createElement("div", { style: { display: "flex", flexDirection: "row", gap: "1rem" } }, /* @__PURE__ */ React57.createElement(
    "input",
    {
      id: "use-e2ee",
      type: "checkbox",
  ...
  ), /* @__PURE__ */ React57.createElement("label", { htmlFor: "use-e2ee" }, "Enable end-to-end encryption")), e2ee && /* @__PURE__ */ React57.createElement("div", { style: { display: "flex", flexDirection: "row", gap: "1rem" } }, /* @__PURE__ */ React57.createElement("label", { htmlFor: "passphrase" }, "Passphrase"), /* @__PURE__ */ React57.createElement(
    "input",
    {
      id: "passphrase",
      type: "password",
    ...
    joinLabel
  )), debug && /* @__PURE__ */ React57.createElement(React57.Fragment, null, /* @__PURE__ */ React57.createElement("strong", null, "User Choices:"), /* @__PURE__ */ React57.createElement("ul", { className: "lk-list", style: { overflow: "hidden", maxWidth: "15rem" } }, /* @__PURE__ */ React57.createElement("li", null, "Username: ", `${userChoices.username}`), /* @__PURE__ */ React57.createElement("li", null, "Video Enabled: ", `${userChoices.videoEnabled}`), /* @__PURE__ */ React57.createElement("li", null, "Audio Enabled: ", `${userChoices.audioEnabled}`), /* @__PURE__ */ React57.createElement("li", null, "Video Device: ", `${userChoices.videoDeviceId}`), /* @__PURE__ */ React57.createElement("li", null, "Audio Device: ", `${userChoices.audioDeviceId}`))));
}
...
function LayoutContextProvider({
  ...
  return /* @__PURE__ */ React58.createElement(LayoutContext.Provider, { value: layoutContextValue }, children);
}
...
function AudioTrack(_a) {
  ...
  const mediaEl = React59.useRef(null);
  ...
  return /* @__PURE__ */ React59.createElement("audio", __spreadValues({ ref: mediaEl }, elementProps));
}
...
function RoomAudioRenderer() {
  ...
  return /* @__PURE__ */ React60.createElement("div", { style: { display: "none" } }, tracks.map((trackRef) => /* @__PURE__ */ React60.createElement(AudioTrack, { key: (0, import_components_core30.getTrackReferenceId)(trackRef), trackRef })));
}
...
function DisconnectButton(props) {
  ...
  return /* @__PURE__ */ React63.createElement("button", __spreadValues({}, buttonProps), props.children);
}
...
function FocusToggle(_a) {
  ...
  return /* @__PURE__ */ React64.createElement(LayoutContext.Consumer, null, (layoutContext) => layoutContext !== void 0 && /* @__PURE__ */ React64.createElement("button", __spreadValues({}, mergedProps), props.children ? props.children : inFocus ? /* @__PURE__ */ React64.createElement(UnfocusToggleIcon_default, null) : /* @__PURE__ */ React64.createElement(FocusToggleIcon_default, null)));
}
...
function ConnectionQualityIndicator(props) {
  ...
  const elementProps = React66.useMemo(() => {
    return __spreadProps(__spreadValues({}, mergeProps2(props, { className })), { "data-lk-quality": quality });
  }, [quality, props, className]);
  return /* @__PURE__ */ React66.createElement("div", __spreadValues({}, elementProps), (_a = props.children) != null ? _a : getConnectionQualityIcon(quality));
}
...
function ParticipantName(_a) {
  ...
  const p = useEnsureParticipant(participant);
  const { className, infoObserver } = React67.useMemo(() => {
    return (0, import_components_core32.setupParticipantName)(p);
  }, [p]);
  ...
  return /* @__PURE__ */ React67.createElement("span", __spreadValues({}, mergedProps), name !== "" ? name : identity, props.children);
}
...
function TrackMutedIndicator(_a) {
  ...
  const htmlProps = React68.useMemo(
    () => mergeProps2(props, {
      className
    }),
    [className, props]
  ...
  return /* @__PURE__ */ React68.createElement("div", __spreadProps(__spreadValues({}, htmlProps), { "data-lk-muted": isMuted }), (_b2 = props.children) != null ? _b2 : getSourceIcon((_a2 = trackRef == null ? void 0 : trackRef.source) != null ? _a2 : source, !isMuted));
}
...
function VideoTrack(_a) {
  ...
  const participant = useEnsureParticipant(_participant);
  const mediaEl = React69.useRef(null);
  ...
  React69.useEffect(() => {
    if (manageSubscription && _publication instanceof import_livekit_client10.RemoteTrackPublication && (debouncedIntersectionEntry == null ? void 0 : debouncedIntersectionEntry.isIntersecting) === false && (intersectionEntry == null ? void 0 : intersectionEntry.isIntersecting) === false) {
      _publication.setSubscribed(false);
    }
  }, [debouncedIntersectionEntry, _publication, manageSubscription]);
  React69.useEffect(() => {
    if (manageSubscription && _publication instanceof import_livekit_client10.RemoteTrackPublication && (intersectionEntry == null ? void 0 : intersectionEntry.isIntersecting) === true) {
  ...
  return /* @__PURE__ */ React69.createElement("video", __spreadProps(__spreadValues({ ref: mediaEl }, elementProps), { muted: true, onClick: clickHandler }));
}
...
function useIsEncrypted(participant) {
  ...
  const observer = React70.useMemo(() => (0, import_components_core33.encryptionStatusObservable)(room, p), [room, p]);
  const isEncrypted = useObservableState(
    observer,
    p instanceof import_livekit_client11.LocalParticipant ? p.isE2EEEnabled : p.isEncrypted
  );
  return isEncrypted;
}
...
function ParticipantContextIfNeeded(props) {
  ...
  return props.participant && !hasContext ? /* @__PURE__ */ React71.createElement(ParticipantContext.Provider, { value: props.participant }, props.children) : /* @__PURE__ */ React71.createElement(React71.Fragment, null, props.children);
}
function TrackRefContextIfNeeded(props) {
  const hasContext = !!useMaybeTrackRefContext();
  return props.trackRef && !hasContext ? /* @__PURE__ */ React71.createElement(TrackRefContext.Provider, { value: props.trackRef }, props.children) : /* @__PURE__ */ React71.createElement(React71.Fragment, null, props.children);
}
function ParticipantTile(_a) {
  ...
  const maybeTrackRef = useMaybeTrackRefContext();
  const p = useEnsureParticipant(participant);
  const trackReference = React71.useMemo(() => {
    var _a3, _b2, _c, _d, _e, _f;
    return {
      participant: (_b2 = (_a3 = trackRef == null ? void 0 : trackRef.participant) != null ? _a3 : maybeTrackRef == null ? void 0 : maybeTrackRef.participant) != null ? _b2 : p,
      source: (_d = (_c = trackRef == null ? void 0 : trackRef.source) != null ? _c : maybeTrackRef == null ? void 0 : maybeTrackRef.source) != null ? _d : source,
      publication: (_f = (_e = trackRef == null ? void 0 : trackRef.publication) != null ? _e : maybeTrackRef == null ? void 0 : maybeTrackRef.publication) != null ? _f : publication
    };
  ...
  const layoutContext = useMaybeLayoutContext();
  const handleSubscribe = React71.useCallback(
    (subscribed) => {
      if (trackReference.source && !subscribed && layoutContext && layoutContext.pin.dispatch && (0, import_components_core34.isTrackReferencePinned)(trackReference, layoutContext.pin.state)) {
        layoutContext.pin.dispatch({ msg: "clear_pin" });
      }
    },
    [trackReference, layoutContext]
  );
  return /* @__PURE__ */ React71.createElement("div", __spreadValues({ style: { position: "relative" } }, elementProps), /* @__PURE__ */ React71.createElement(TrackRefContextIfNeeded, { trackRef: trackReference }, /* @__PURE__ */ React71.createElement(ParticipantContextIfNeeded, { participant: trackReference.participant }, children != null ? children : /* @__PURE__ */ React71.createElement(React71.Fragment, null, (0, import_components_core34.isTrackReference)(trackReference) && (((_a2 = trackReference.publication) == null ? void 0 : _a2.kind) === "video" || trackReference.source === import_livekit_client12.Track.Source.Camera || trackReference.source === import_livekit_client12.Track.Source.ScreenShare) ? /* @__PURE__ */ React71.createElement(
    VideoTrack,
    {
  ...
  ) : (0, import_components_core34.isTrackReference)(trackReference) && /* @__PURE__ */ React71.createElement(
    AudioTrack,
    {
      trackRef: trackReference,
      onSubscriptionStatusChanged: handleSubscribe
    }
  ), /* @__PURE__ */ React71.createElement("div", { className: "lk-participant-placeholder" }, /* @__PURE__ */ React71.createElement(ParticipantPlaceholder_default, null)), /* @__PURE__ */ React71.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React71.createElement("div", { className: "lk-participant-metadata-item" }, trackReference.source === import_livekit_client12.Track.Source.Camera ? /* @__PURE__ */ React71.createElement(React71.Fragment, null, isEncrypted && /* @__PURE__ */ React71.createElement(LockLockedIcon_default, { style: { marginRight: "0.25rem" } }), /* @__PURE__ */ React71.createElement(
    TrackMutedIndicator,
    {
      source: import_livekit_client12.Track.Source.Microphone,
      show: "muted"
    }
  ), /* @__PURE__ */ React71.createElement(ParticipantName, null)) : /* @__PURE__ */ React71.createElement(React71.Fragment, null, /* @__PURE__ */ React71.createElement(ScreenShareIcon_default, { style: { marginRight: "0.25rem" } }), /* @__PURE__ */ React71.createElement(ParticipantName, null, "'s screen"))), /* @__PURE__ */ React71.createElement(ConnectionQualityIndicator, { className: "lk-participant-metadata-item" }))), /* @__PURE__ */ React71.createElement(FocusToggle, { trackRef: trackReference }))));
}

// src/components/layout/FocusLayout.tsx
function FocusLayoutContainer(props) {
  const elementProps = mergeProps2(props, { className: "lk-focus-layout" });
  return /* @__PURE__ */ React72.createElement("div", __spreadValues({}, elementProps), props.children);
}
function FocusLayout(_a) {
  ...
  return /* @__PURE__ */ React72.createElement(ParticipantTile, __spreadValues(__spreadValues({}, trackReference), htmlProps));
}
...
function TrackLoop(_a) {
  ...
  return /* @__PURE__ */ React73.createElement(React73.Fragment, null, tracks.map((trackReference) => {
    return /* @__PURE__ */ React73.createElement(
      TrackRefContext.Provider,
      {
        value: trackReference,
        key: (0, import_components_core35.getTrackReferenceId)(trackReference)
      },
...
function PaginationControl({
  ...
  const [interactive, setInteractive] = React74.useState(false);
  React74.useEffect(() => {
    let subscription;
    if (connectedElement) {
      subscription = (0, import_components_core36.createInteractingObservable)(connectedElement.current, 2e3).subscribe(
        setInteractive
      );
    }
    return () => {
  ...
  return /* @__PURE__ */ React74.createElement("div", { className: "lk-pagination-control", "data-lk-user-interaction": interactive }, /* @__PURE__ */ React74.createElement("button", { className: "lk-button", onClick: prevPage }, /* @__PURE__ */ React74.createElement(Chevron_default, null)), /* @__PURE__ */ React74.createElement("span", { className: "lk-pagination-count" }, `${currentPage} of ${totalPageCount}`), /* @__PURE__ */ React74.createElement("button", { className: "lk-button", onClick: nextPage }, /* @__PURE__ */ React74.createElement(Chevron_default, null)));
}
...
function PaginationIndicator({ totalPageCount, currentPage }) {
  const bubbles = new Array(totalPageCount).fill("").map((_, index) => {
    if (index + 1 === currentPage) {
      return /* @__PURE__ */ React75.createElement("span", { "data-lk-active": true, key: index });
    } else {
      return /* @__PURE__ */ React75.createElement("span", { key: index });
    }
  });
  return /* @__PURE__ */ React75.createElement("div", { className: "lk-pagination-indicator" }, bubbles);
}

// src/components/layout/GridLayout.tsx
function GridLayout(_a) {
  var _b = _a, { tracks } = _b, props = __objRest(_b, ["tracks"]);
  const gridEl = React76.createRef();
  ...
  return /* @__PURE__ */ React76.createElement("div", __spreadValues({ ref: gridEl, "data-lk-pagination": pagination.totalPageCount > 1 }, elementProps), /* @__PURE__ */ React76.createElement(TrackLoop, { tracks: pagination.tracks }, props.children), tracks.length > layout.maxTiles && /* @__PURE__ */ React76.createElement(React76.Fragment, null, /* @__PURE__ */ React76.createElement(
    PaginationIndicator,
    {
      totalPageCount: pagination.totalPageCount,
      currentPage: pagination.currentPage
    }
  ), /* @__PURE__ */ React76.createElement(PaginationControl, __spreadValues({ pagesContainer: gridEl }, pagination))));
}
...
function CarouselLayout(_a) {
  ...
  const asideEl = React77.useRef(null);
  const [prevTiles, setPrevTiles] = React77.useState(0);
  ...
  const scrollBarWidth = (0, import_components_core37.getScrollBarWidth)();
  ...
  return /* @__PURE__ */ React77.createElement("aside", __spreadValues({ key: carouselOrientation, className: "lk-carousel", ref: asideEl }, props), /* @__PURE__ */ React77.createElement(TrackLoop, { tracks: sortedTiles }, props.children));
}
...
function AudioVisualizer(_a) {
  ...
  if (!ref) {
    ...
    ref = { participant: p, source: import_livekit_client13.Track.Source.Microphone };
  }
...
function ParticipantAudioTile(_a) {
  ...
  return /* @__PURE__ */ React81.createElement("div", __spreadValues({ style: { position: "relative" } }, elementProps), /* @__PURE__ */ React81.createElement(ParticipantContextIfNeeded, { participant: p }, children != null ? children : /* @__PURE__ */ React81.createElement(React81.Fragment, null, /* @__PURE__ */ React81.createElement(AudioTrack, { source: source != null ? source : import_livekit_client15.Track.Source.Microphone }), /* @__PURE__ */ React81.createElement(AudioVisualizer, null), /* @__PURE__ */ React81.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React81.createElement("div", { className: "lk-participant-metadata-item" }, /* @__PURE__ */ React81.createElement(TrackMutedIndicator, { source: import_livekit_client15.Track.Source.Microphone }), /* @__PURE__ */ React81.createElement(ParticipantName, null)), /* @__PURE__ */ React81.createElement(ConnectionQualityIndicator, { className: "lk-participant-metadata-item" })))));
}
...
function HostEndMeetingMenu(_a) {
  ...
  const [updateRequired, setUpdateRequired] = React83.useState(true);
  const room = useRoomContext();
  const button = React83.useRef(null);
  const leaveButtonRef = React83.useRef(null);
  const tooltip = React83.useRef(null);
  const { disconnect } = (0, import_components_core38.setupDisconnectButton)(room);
  const [showDropdown, setShowDropdown] = React83.useState(false);
  ...
  React83.useLayoutEffect(() => {
    if (button.current && tooltip.current && updateRequired) {
      (0, import_components_core38.computeMenuPosition)(button.current, tooltip.current).then(({ x, y }) => {
        console.log(x, y);
        if (tooltip.current) {
  ...
  const handleClickOutside = React83.useCallback(
    (event) => {
      if (!tooltip.current) {
        return;
      }
      ...
      if (isOpen && (0, import_components_core38.wasClickOutside)(tooltip.current, event)) {
        setIsOpen(false);
  ...
  return /* @__PURE__ */ React83.createElement(React83.Fragment, null, /* @__PURE__ */ React83.createElement(
    ...
    /* @__PURE__ */ React83.createElement("ul", { className: "lk-media-device-select lk-list", style: { display: !showDropdown ? "unset" : "none" } }, endForAll && /* @__PURE__ */ React83.createElement("li", null, /* @__PURE__ */ React83.createElement(DisconnectButton, { onClick: endMeeting }, endForAll)), leave && /* @__PURE__ */ React83.createElement("li", null, /* @__PURE__ */ React83.createElement("button", { ref: leaveButtonRef, className: "lk-disconnect-button", onClick: handleLeave }, "Leave Meeting"))),
    showDropdown && /* @__PURE__ */ React83.createElement("div", { className: "assign-menu" }, /* @__PURE__ */ React83.createElement("select", { value, onChange: handleChangeValue }, remoteParticipants.map((participant) => {
      var _a2;
      return /* @__PURE__ */ React83.createElement("option", { value: participant.identity, key: participant.identity }, (_a2 = participant == null ? void 0 : participant.name) == null ? void 0 : _a2.substring(0, 10));
    })), /* @__PURE__ */ React83.createElement("div", { className: "button-container" }, /* @__PURE__ */ React83.createElement("button", { className: "lk-button", onClick: handleCancel }, "Cancel"), /* @__PURE__ */ React83.createElement("button", { className: "lk-button", onClick: handleChange }, "Ok"))),
    /* @__PURE__ */ React83.createElement("div", { className: "arrow" }, /* @__PURE__ */ React83.createElement("div", { className: "arrow-shape" }))
  ));
}
...
function useToggleUserLink({ props }) {
  const { dispatch, state } = useLayoutContext().widget;
  const { className } = React84.useMemo(() => (0, import_components_core39.setupUserToggle)(), []);
  const mergedProps = React84.useMemo(
    () => mergeProps2(props, {
      className,
      onClick: () => {
        if (dispatch)
...
function UserToggle(props) {
  ...
  return /* @__PURE__ */ React84.createElement("button", __spreadValues({}, mergedProps), props.children);
}

// src/assets/icons/InviteIcon.tsx
var React85 = __toESM(require("react"));
var SvgInviteIcon = (props) => /* @__PURE__ */ React85.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 25, height: 20, fill: "none" }, props), /* @__PURE__ */ React85.createElement(
  "path",
  {
...
var SvgUserIcon = (props) => /* @__PURE__ */ React86.createElement("svg", __spreadValues({ xmlns: "http://www.w3.org/2000/svg", width: 20, height: 20, fill: "none" }, props), /* @__PURE__ */ React86.createElement(
  "path",
  {
    id: "Vector",
...
function ControlBar(_a) {
  ...
  var _a2, _b2, _c, _d, _e, _f, _g;
  const layoutContext = useMaybeLayoutContext();
  const [isChatOpen, setIsChatOpen] = React87.useState(false);
  const [isShareLinkOpen, setIsShareLinkOpen] = React87.useState(false);
  const [isUserOpen, setIsUserOpen] = React87.useState(false);
  ...
  const showIcon = React87.useMemo(
    () => variation === "minimal" || variation === "verbose",
    [variation]
  );
  const showText = React87.useMemo(
    () => variation === "textOnly" || variation === "verbose",
    [variation]
  );
  const browserSupportsScreenSharing = (0, import_components_core40.supportsScreenSharing)();
  const [isScreenShareEnabled, setIsScreenShareEnabled] = React87.useState(false);
  const onScreenShareChange = (enabled) => {
    setIsScreenShareEnabled(enabled);
  };
  const htmlProps = mergeProps2({ className: "lk-control-bar" }, props);
  return /* @__PURE__ */ React87.createElement("div", __spreadValues({}, htmlProps), visibleControls.microphone && /* @__PURE__ */ React87.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React87.createElement(TrackToggle, { source: import_livekit_client16.Track.Source.Microphone, showIcon }, showText && "Microphone"), /* @__PURE__ */ React87.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React87.createElement(MediaDeviceMenu, { kind: "audioinput" }))), visibleControls.camera && /* @__PURE__ */ React87.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React87.createElement(TrackToggle, { source: import_livekit_client16.Track.Source.Camera, showIcon }, showText && "Camera"), /* @__PURE__ */ React87.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React87.createElement(MediaDeviceMenu, { kind: "videoinput" }))), visibleControls.screenShare && browserSupportsScreenSharing && /* @__PURE__ */ React87.createElement(
    TrackToggle,
    {
  ...
  ), visibleControls.chat && /* @__PURE__ */ React87.createElement(ChatToggle, null, showIcon && /* @__PURE__ */ React87.createElement(ChatIcon_default, null), showText && "Chat", state && state.unreadMessages !== 0 && /* @__PURE__ */ React87.createElement("span", { className: "waiting-count" }, state.unreadMessages < 10 ? state.unreadMessages.toFixed(0) : "9+")), visibleControls.sharelink && /* @__PURE__ */ React87.createElement(ShareLinkToggle, null, showIcon && /* @__PURE__ */ React87.createElement(InviteIcon_default, null), showText && "Invite"), visibleControls.users && /* @__PURE__ */ React87.createElement(UserToggle, null, showIcon && /* @__PURE__ */ React87.createElement(UsersIcon_default, null), showText && "Participants", waitingRoomCount !== 0 && /* @__PURE__ */ React87.createElement("span", { className: "waiting-count" }, waitingRoomCount)), visibleControls.endForAll ? /* @__PURE__ */ React87.createElement("div", { className: "tl-leave lk-button-group" }, /* @__PURE__ */ React87.createElement("button", { className: "lk-disconnect-button" }, showIcon && /* @__PURE__ */ React87.createElement(LeaveIcon_default, null), showText && "Leave Meeting"), /* @__PURE__ */ React87.createElement("div", { className: "tl-leave-btn lk-button-group-menu" }, /* @__PURE__ */ React87.createElement(
    HostEndMeetingMenu,
    {
  ...
  ))) : /* @__PURE__ */ React87.createElement(DisconnectButton, null, showIcon && /* @__PURE__ */ React87.createElement(LeaveIcon_default, null), showText && visibleControls.leaveButton), /* @__PURE__ */ React87.createElement(StartAudio, { label: "Start Audio" }));
}
...
var ParticipantList = (_a) => {
  ...
  return /* @__PURE__ */ React88.createElement("div", __spreadValues({ style: { position: "relative" } }, elementProps), /* @__PURE__ */ React88.createElement(ParticipantContextIfNeeded, { participant: p }, children != null ? children : /* @__PURE__ */ React88.createElement(React88.Fragment, null, /* @__PURE__ */ React88.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React88.createElement("div", { className: "lk-participant-metadata-item" }, /* @__PURE__ */ React88.createElement(ParticipantName, null)), /* @__PURE__ */ React88.createElement("div", { className: "display-flex" }, /* @__PURE__ */ React88.createElement(
    TrackMutedIndicator,
    {
      source: import_livekit_client17.Track.Source.Microphone,
      show: "always"
...
var SvgApproveIcon = (props) => /* @__PURE__ */ React90.createElement(
  "svg",
  __spreadValues({
    width: "15",
    height: "11",
...
var SvgRejectIcon = (props) => /* @__PURE__ */ React91.createElement(
  "svg",
  __spreadValues({
    width: "15",
    height: "11",
...
function Users(_a) {
  ...
  const ulRef = React92.useRef(null);
  const participants = useParticipants();
  const [waitingRoom, setWaitingRoom] = React92.useState([]);
  const [toggleWaiting, setToggleWaiting] = React92.useState(true);
  const room = useRoomContext();
  function usersList() {
    return __async(this, null, function* () {
      const postData = {
  ...
  return /* @__PURE__ */ React92.createElement("div", __spreadProps(__spreadValues({}, props), { className: "lk-chat lk-users" }), /* @__PURE__ */ React92.createElement("div", { className: "lk-waitinroom" }, /* @__PURE__ */ React92.createElement("div", { className: "tl-waitingroom-heading" }, /* @__PURE__ */ React92.createElement("h3", null, "Waiting Room"), /* @__PURE__ */ React92.createElement("div", { className: "tl-toggle-switch" }, /* @__PURE__ */ React92.createElement(
    ToggleSwitch,
    {
      id: "toggleSwitch",
  ...
  )), toggleWaiting && waitingRoom.length ? /* @__PURE__ */ React92.createElement(
    "button",
    {
    ...
    "Approve All"
  ) : ""), waitingRoom.map((item) => /* @__PURE__ */ React92.createElement("div", { className: "tl-participant-li", key: item.username }, /* @__PURE__ */ React92.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React92.createElement("div", { className: "lk-participant-metadata-item" }, item.username), /* @__PURE__ */ React92.createElement("div", { className: "display-flex" }, /* @__PURE__ */ React92.createElement(
    "button",
    {
      className: "lk-button lk-waiting-room lk-success",
      onClick: () => admitUser(item.identity, "accepted")
    },
    /* @__PURE__ */ React92.createElement(ApproveIcon_default, null)
    ...
    /* @__PURE__ */ React92.createElement(RejectIcon_default, null)
  )))))), /* @__PURE__ */ React92.createElement("div", { className: "lk-participants" }, /* @__PURE__ */ React92.createElement("div", { className: "tl-participants-heading" }, /* @__PURE__ */ React92.createElement("h3", null, "Participants  ", /* @__PURE__ */ React92.createElement("span", null, "(", participants.length, ")"))), (participants == null ? void 0 : participants.length) ? /* @__PURE__ */ React92.createElement(ParticipantLoop, { participants }, /* @__PURE__ */ React92.createElement(ParticipantList, null)) : /* @__PURE__ */ React92.createElement("div", null, /* @__PURE__ */ React92.createElement("h5", null, "No Participants"))));
}
...
function VideoConference(_a) {
  ...
  var _a2, _b2;
  const [widgetState, setWidgetState] = React93.useState({
    showChat: null,
    unreadMessages: 0
  });
  const lastAutoFocusedScreenShareTrack = React93.useRef(null);
  const { localParticipant } = useLocalParticipant();
  const p = useEnsureParticipant(localParticipant);
  const { infoObserver } = React93.useMemo(() => {
    return (0, import_components_core41.setupParticipantName)(p);
  }, [p]);
  ...
  const [showShareButton, setShowShareButton] = React93.useState(false);
  const [showParticipantButton, setShowParticipantButton] = React93.useState(false);
  const [leaveButton, setLeaveButton] = React93.useState("Leave");
  const [endForAll, setEndForAll] = React93.useState(false);
  const meta = metadata ? JSON.parse(metadata) : {};
  const [waiting, setWaiting] = React93.useState(null);
  const [waitingRoomCount, setWaitingRoomCount] = React93.useState(0);
  ...
  const updateCount = (count) => {
    import_components_core41.log.debug("count ", count);
  ...
  const screenShareTracks = tracks.filter(import_components_core41.isTrackReference).filter((track) => track.publication.source === import_livekit_client18.Track.Source.ScreenShare);
  ...
  React93.useEffect(() => {
    ...
    if (screenShareTracks.some((track) => track.publication.isSubscribed) && lastAutoFocusedScreenShareTrack.current === null) {
      import_components_core41.log.debug("Auto set screen share focus:", { newScreenShareTrack: screenShareTracks[0] });
      ...
      import_components_core41.log.debug("Auto clearing screen share focus.");
  ...
  return /* @__PURE__ */ React93.createElement("div", __spreadValues({ className: "lk-video-conference" }, props), (0, import_components_core41.isWeb)() && /* @__PURE__ */ React93.createElement(
    LayoutContextProvider,
    {
      value: layoutContext,
      onWidgetChange: widgetUpdate
    },
    /* @__PURE__ */ React93.createElement("div", { className: "lk-video-conference-inner" }, !focusTrack ? /* @__PURE__ */ React93.createElement("div", { className: "lk-grid-layout-wrapper" }, /* @__PURE__ */ React93.createElement(GridLayout, { tracks }, /* @__PURE__ */ React93.createElement(ParticipantTile, null))) : /* @__PURE__ */ React93.createElement("div", { className: "lk-focus-layout-wrapper" }, /* @__PURE__ */ React93.createElement(FocusLayoutContainer, null, /* @__PURE__ */ React93.createElement(CarouselLayout, { tracks: carouselTracks }, /* @__PURE__ */ React93.createElement(ParticipantTile, null)), focusTrack && /* @__PURE__ */ React93.createElement(FocusLayout, { trackRef: focusTrack }))), /* @__PURE__ */ React93.createElement(
      ControlBar,
      {
        controls: {
          chat: true,
    ...
    showShareButton ? /* @__PURE__ */ React93.createElement(ShareLink, { style: {
      display: widgetState.showChat == "show_invite" ? "flex" : "none"
    } }) : /* @__PURE__ */ React93.createElement(React93.Fragment, null),
    showParticipantButton ? /* @__PURE__ */ React93.createElement(
      Users,
      {
    ...
    waiting ? /* @__PURE__ */ React93.createElement(Toast, { className: "lk-toast-connection-state" }, /* @__PURE__ */ React93.createElement(UserToggle, null, waiting)) : /* @__PURE__ */ React93.createElement(React93.Fragment, null),
  ...
  ), /* @__PURE__ */ React93.createElement(RoomAudioRenderer, null), /* @__PURE__ */ React93.createElement(ConnectionStateToast, null));
}
...
function AudioConference(_a) {
  var props = __objRest(_a, []);
  const [widgetState, setWidgetState] = React94.useState({
    showChat: null,
    unreadMessages: 0
  });
  const participants = useParticipants();
  return /* @__PURE__ */ React94.createElement(LayoutContextProvider, { onWidgetChange: setWidgetState }, /* @__PURE__ */ React94.createElement("div", __spreadValues({ className: "lk-audio-conference" }, props), /* @__PURE__ */ React94.createElement("div", { className: "lk-audio-conference-stage" }, /* @__PURE__ */ React94.createElement(ParticipantLoop, { participants }, /* @__PURE__ */ React94.createElement(ParticipantAudioTile, null))), /* @__PURE__ */ React94.createElement(
    ControlBar,
    {
      controls: { microphone: true, screenShare: false, camera: false, chat: true },
      waitingRoomCount: 0
    }
  ), widgetState.showChat == "show_chat" && /* @__PURE__ */ React94.createElement(Chat, null)));
}
