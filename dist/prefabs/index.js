...
function UserChat(_a) {
  ...
  return /* @__PURE__ */ React9.createElement(
    ...
    /* @__PURE__ */ React9.createElement("span", { className: "lk-meta-data" }, /* @__PURE__ */ React9.createElement("strong", { className: "lk-participant-name" }, (_d = (_b2 = entry.from) == null ? void 0 : _b2.name) != null ? _d : (_c = entry.from) == null ? void 0 : _c.identity)),
...
function Chat(_a) {
  ...
  return /* @__PURE__ */ React10.createElement("div", __spreadProps(__spreadValues({}, props), { className: "lk-chat" }), /* @__PURE__ */ React10.createElement("ul", { className: "tl-list lk-chat-messages", ref: ulRef }, props.children ? chatMessages.reverse().map(
...
function useConnectionQualityIndicator(options = {}) {
  ...
  const { className, connectionQualityObserver } = React13.useMemo(
    () => (0, import_components_core3.setupConnectionQualityIndicator)(p),
    [p]
  );
...
function useConnectionState(room) {
  ...
  const observable = React14.useMemo(() => (0, import_components_core4.connectionStateObserver)(r), [r]);
...
function useFocusToggle({ trackRef, trackSource, participant, props }) {
  ...
  const layoutContext = useMaybeLayoutContext();
  const { className } = React16.useMemo(() => (0, import_components_core6.setupFocusToggle)(), []);
  const inFocus = React16.useMemo(() => {
    if (trackRef) {
      return (0, import_components_core6.isTrackReferencePinned)(trackRef, layoutContext == null ? void 0 : layoutContext.pin.state);
    } else if (trackSource) {
  ...
  const mergedProps = React16.useMemo(
    () => mergeProps(props, {
      ...
      onClick: (event) => {
        ...
        if (trackRef && (0, import_components_core6.isTrackReference)(trackRef)) {
          if (inFocus) {
...
function useGridLayout(gridElement, trackCount) {
  ...
  const layout = width > 0 && height > 0 ? (0, import_components_core7.selectGridLayout)(import_components_core7.GRID_LAYOUTS, trackCount, width, height) : import_components_core7.GRID_LAYOUTS[0];
  React17.useEffect(() => {
...
function useIsMuted(sourceOrTrackRef, options = {}) {
  ...
  React18.useEffect(() => {
    const listener = (0, import_components_core8.mutedObserver)(ref).subscribe(setIsMuted);
    return () => listener.unsubscribe();
  }, [(0, import_components_core8.getTrackReferenceId)(ref)]);
  return isMuted;
}
...
function useIsSpeaking(participant) {
  const p = useEnsureParticipant(participant);
  const observable = React19.useMemo(() => (0, import_components_core9.createIsSpeakingObserver)(p), [p]);
...
function useLocalParticipant(options = {}) {
  ...
  React20.useEffect(() => {
    const listener = (0, import_components_core10.observeParticipantMedia)(localParticipant).subscribe(handleUpdate);
    return () => listener.unsubscribe();
  }, [localParticipant]);
  return {
    isMicrophoneEnabled,
...
function useLocalParticipantPermissions() {
  ...
  const permissionObserver = React21.useMemo(
    () => (0, import_components_core11.participantPermissionObserver)(room.localParticipant),
    [room]
  );
...
function useMediaDeviceSelect({
  ...
  const deviceObserver = React22.useMemo(
    () => (0, import_components_core12.createMediaDeviceObserver)(kind, requestPermissions),
  ...
  const { className, activeDeviceObservable, setActiveMediaDevice } = React22.useMemo(
    () => (0, import_components_core12.setupDeviceSelector)(kind, room != null ? room : roomContext, track),
    [kind, room, roomContext, track]
  );
  React22.useEffect(() => {
    const listener = activeDeviceObservable.subscribe((deviceId) => {
      import_components_core12.log.info("setCurrentDeviceId", deviceId);
      if (deviceId)
...
function useMediaDevices({ kind }) {
  const deviceObserver = React23.useMemo(() => (0, import_components_core13.createMediaDeviceObserver)(kind), [kind]);
  const devices = useObservableState(deviceObserver, []);
  return devices;
}
...
function useMediaTrackBySourceOrName(observerOptions, options = {}) {
  var _a;
  const [publication, setPublication] = React24.useState((0, import_components_core15.getTrackByIdentifier)(observerOptions));
  ...
  const { className, trackObserver } = React24.useMemo(() => {
    return (0, import_components_core15.setupMediaTrack)(observerOptions);
  }, [
    (_a = observerOptions.participant.sid) != null ? _a : observerOptions.participant.identity,
    observerOptions.source,
    (0, import_components_core14.isTrackReference)(observerOptions) && observerOptions.publication.trackSid
  ]);
  React24.useEffect(() => {
    const subscription = trackObserver.subscribe((publication2) => {
      import_components_core15.log.debug("update track", publication2);
  ...
  React24.useEffect(() => {
    ...
    if (track) {
      ...
      if (((_a2 = options.element) == null ? void 0 : _a2.current) && !((0, import_components_core15.isLocal)(observerOptions.participant) && (track == null ? void 0 : track.kind) === "audio")) {
...
function useVisualStableUpdate(trackReferences, maxItemsOnPage, options = {}) {
  ...
  const sortedTrackRefs = typeof options.customSortFunction === "function" ? options.customSortFunction(trackReferences) : (0, import_components_core16.sortTrackReferences)(trackReferences);
  let updatedTrackRefs = [...sortedTrackRefs];
  if (layoutChanged === false) {
    try {
      updatedTrackRefs = (0, import_components_core16.updatePages)(lastTrackRefs.current, sortedTrackRefs, maxItemsOnPage);
    } catch (error) {
      import_components_core16.log.error("Error while running updatePages(): ", error);
    }
  }
  if (layoutChanged) {
...
function useParticipantTile({
  ...
  const mergedProps = React27.useMemo(() => {
    const { className } = (0, import_components_core17.setupParticipantTile)();
...
function useChatToggle({ props }) {
  ...
  const { className } = React32.useMemo(() => (0, import_components_core20.setupChatToggle)(), []);
...
function useTrackMutedIndicator(trackRefOrSource, options = {}) {
  ...
  const { className, mediaMutedObserver } = React33.useMemo(
    () => (0, import_components_core21.setupTrackMutedIndicator)(ref),
    [(0, import_components_core21.getTrackReferenceId)(ref)]
  );
...
function useTrackToggle(_a) {
  ...
  const { toggle, className, pendingObserver, enabledObserver } = React34.useMemo(
    () => room ? (0, import_components_core22.setupMediaToggle)(source, room, captureOptions) : (0, import_components_core22.setupManualToggle)(),
  ...
  React34.useEffect(() => {
    if (initialState !== void 0) {
      import_components_core22.log.debug("forcing initial toggle state", source, initialState);
      toggle(initialState);
    }
  }, []);
...
function useTracks(sources = [
  ...
  const sources_ = React35.useMemo(() => {
    return sources.map((s) => (0, import_components_core23.isSourceWitOptions)(s) ? s.source : s);
  }, [JSON.stringify(sources)]);
  React35.useEffect(() => {
    const subscription = (0, import_components_core23.trackReferencesObservable)(room, sources_, {
      additionalRoomEvents: options.updateOnlyOn,
      onlySubscribed: options.onlySubscribed
    }).subscribe(({ trackReferences: trackReferences2, participants: participants2 }) => {
      import_components_core23.log.debug("setting track bundles", trackReferences2, participants2);
  ...
  const maybeTrackReferences = React35.useMemo(() => {
    if ((0, import_components_core23.isSourcesWithOptions)(sources)) {
      ...
      participants.forEach((participant) => {
        ...
        if (requirePlaceholder.has(participant.identity)) {
          ...
          sourcesToAddPlaceholder.forEach((placeholderSource) => {
            ...
            import_components_core23.log.debug(
...
function MediaDeviceMenu(_a) {
  ...
  const handleActiveDeviceChange = (kind2, deviceId) => {
    import_components_core25.log.debug("handle device change");
    setIsOpen(false);
  ...
  React37.useLayoutEffect(() => {
    if (button.current && tooltip.current && (devices || updateRequired)) {
      (0, import_components_core24.computeMenuPosition)(button.current, tooltip.current).then(({ x, y }) => {
        if (tooltip.current) {
  ...
  const handleClickOutside = React37.useCallback(
    (event) => {
      ...
      if (isOpen && (0, import_components_core24.wasClickOutside)(tooltip.current, event)) {
        setIsOpen(false);
      }
    },
...
function usePreviewDevice(enabled, deviceId, kind) {
  ...
  React58.useEffect(() => {
    if (enabled && !localTrack && !deviceError && !isCreatingTrack) {
      import_components_core26.log.debug("creating track", kind);
  ...
  React58.useEffect(() => {
    ...
    if (!enabled) {
      import_components_core26.log.debug(`muting ${kind} track`);
      localTrack.mute().then(() => import_components_core26.log.debug(localTrack.mediaStreamTrack));
      ...
      import_components_core26.log.debug(`switching ${kind} device from`, prevDeviceId.current, selectedDevice.deviceId);
      switchDevice(localTrack, selectedDevice.deviceId);
    } else {
      import_components_core26.log.debug(`unmuting local ${kind} track`);
      localTrack.unmute();
    }
  ...
  React58.useEffect(() => {
    return () => {
      if (localTrack) {
        import_components_core26.log.debug(`stopping local ${kind} track`);
        localTrack.stop();
        localTrack.mute();
      }
    };
...
function PreJoin(_a) {
  ...
  function handleSubmit(event) {
    ...
    if (handleValidation(userChoices)) {
      ...
      import_components_core26.log.warn("Validation failed with: ", userChoices);
    }
  }
...
function useToggleShareLink({ props }) {
  const { dispatch, state } = useLayoutContext().widget;
  const { className } = React60.useMemo(() => (0, import_components_core27.setupShareLinkToggle)(), []);
...
function useIsEncrypted(participant) {
  ...
  const observer = React69.useMemo(() => (0, import_components_core30.encryptionStatusObservable)(room, p), [room, p]);
...
function ParticipantTile(_a) {
  ...
  return /* @__PURE__ */ React70.createElement("div", __spreadValues({ style: { position: "relative" } }, elementProps), /* @__PURE__ */ React70.createElement(TrackRefContextIfNeeded, { trackRef: trackReference }, /* @__PURE__ */ React70.createElement(ParticipantContextIfNeeded, { participant: trackReference.participant }, children != null ? children : /* @__PURE__ */ React70.createElement(React70.Fragment, null, (0, import_components_core31.isTrackReference)(trackReference) && (((_b2 = trackReference.publication) == null ? void 0 : _b2.kind) === "video" || trackReference.source === import_livekit_client11.Track.Source.Camera || trackReference.source === import_livekit_client11.Track.Source.ScreenShare) ? /* @__PURE__ */ React70.createElement(
    VideoTrack,
    {
  ...
  ) : (0, import_components_core31.isTrackReference)(trackReference) && /* @__PURE__ */ React70.createElement(
    AudioTrack,
    {
      trackRef: trackReference,
      onSubscriptionStatusChanged: handleSubscribe
    }
  ), /* @__PURE__ */ React70.createElement("div", { className: "lk-participant-placeholder" }, /* @__PURE__ */ React70.createElement(ParticipantPlaceholder_default, null)), /* @__PURE__ */ React70.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React70.createElement("div", { className: "lk-participant-metadata-item" }, trackReference.source === import_livekit_client11.Track.Source.Camera ? /* @__PURE__ */ React70.createElement(React70.Fragment, null, isEncrypted && /* @__PURE__ */ React70.createElement(LockLockedIcon_default, { style: { marginRight: "0.25rem" } }), /* @__PURE__ */ React70.createElement(
    TrackMutedIndicator,
    {
...
function LayoutContextProvider({
  ...
  React77.useEffect(() => {
    import_components_core35.log.debug("Chat Widget Updated", { widgetState: layoutContextValue.widget.state });
...
function RoomAudioRenderer() {
  ...
  return /* @__PURE__ */ React80.createElement("div", { style: { display: "none" } }, tracks.map((trackRef) => /* @__PURE__ */ React80.createElement(AudioTrack, { key: (0, import_components_core36.getTrackReferenceId)(trackRef), trackRef })));
}
...
function formatChatMessageLinks(message) {
  return (0, import_components_core37.tokenize)(message, (0, import_components_core37.createDefaultGrammar)()).map((tok, i) => {
    if (typeof tok === `string`) {
      ...
      return /* @__PURE__ */ React84.createElement("a", { className: "lk-chat-link", key: i, href, target: "_blank", rel: "noreferrer" }, content);
    }
  });
}
...
function ShareLink(_a) {
  ...
  const [users, setUsers] = React85.useState([]);
  const [searched, setSearched] = React85.useState([]);
  const [showToast, setShowToast] = React85.useState(false);
  ...
  const { infoObserver } = React85.useMemo(() => {
  ...
  const [showInviteUser, setShowInviteUser] = React85.useState(true);
  React85.useEffect(() => {
  ...
  return /* @__PURE__ */ React85.createElement("div", __spreadProps(__spreadValues({}, props), { className: "lk-chat lk-sharelink" }), /* @__PURE__ */ React85.createElement("form", { className: "lk-chat-form" }, /* @__PURE__ */ React85.createElement("input", { className: "lk-form-control lk-chat-form-input", type: "text", value: link, readOnly: true }), /* @__PURE__ */ React85.createElement("button", { type: "button", className: "lk-button lk-chat-form-button", onClick: handleCopy }, "Copy")), showToast ? /* @__PURE__ */ React85.createElement(Toast, { className: "lk-toast-connection-state" }, "Copied") : /* @__PURE__ */ React85.createElement(React85.Fragment, null), showInviteUser ? /* @__PURE__ */ React85.createElement("form", { className: "lk-chat-form", onSubmit: handleSubmit }, /* @__PURE__ */ React85.createElement(
    "input",
    {
      className: "lk-form-control lk-chat-form-input",
      ref: inputRef,
  ...
  )) : /* @__PURE__ */ React85.createElement(React85.Fragment, null), showInviteUser && searched.length > 0 ? /* @__PURE__ */ React85.createElement("ul", { className: "lk-list lk-chat-messages", ref: ulRef }, searched.map((user, index) => {
    return /* @__PURE__ */ React85.createElement("li", { key: index, className: "lk-chat-entry" }, /* @__PURE__ */ React85.createElement("div", null, /* @__PURE__ */ React85.createElement("span", { className: "lk-message-body" }, user.full_name), /* @__PURE__ */ React85.createElement("span", { className: "lk-message-body lk-message-text" }, user.user_name)), /* @__PURE__ */ React85.createElement("button", { type: "button", onClick: () => handleInvite(user), className: "lk-button lk-chat-form-button" + (user.invited ? " invited" : "") }, user.invited ? "Invited" : "Invite"));
  })) : "");
}
...
function HostEndMeetingMenu(_a) {
  ...
  const [updateRequired, setUpdateRequired] = React86.useState(true);
  const room = useRoomContext();
  const button = React86.useRef(null);
  const leaveButtonRef = React86.useRef(null);
  const tooltip = React86.useRef(null);
  const { disconnect } = (0, import_components_core39.setupDisconnectButton)(room);
  const [showDropdown, setShowDropdown] = React86.useState(false);
  const [value, setValue] = React86.useState("");
  ...
  const handleClickOutside = React86.useCallback(
    (event) => {
      if (!tooltip.current) {
        return;
      }
  ...
  const [hostError, setHostError] = React86.useState(false);
  const handleChange = () => {
    if (value !== "") {
      setIsOpen(false);
      setValue(value);
  ...
  return /* @__PURE__ */ React86.createElement(React86.Fragment, null, /* @__PURE__ */ React86.createElement(
    ...
    /* @__PURE__ */ React86.createElement("ul", { className: "lk-media-device-select lk-list", style: { display: !showDropdown ? "unset" : "none" } }, endForAll && /* @__PURE__ */ React86.createElement("li", null, /* @__PURE__ */ React86.createElement(DisconnectButton, { onClick: endMeeting }, endForAll)), leave && /* @__PURE__ */ React86.createElement("li", null, /* @__PURE__ */ React86.createElement("button", { ref: leaveButtonRef, className: "lk-disconnect-button", onClick: handleLeave }, "Leave Meeting"))),
    showDropdown && /* @__PURE__ */ React86.createElement("div", { className: "assign-menu" }, hostError && /* @__PURE__ */ React86.createElement("span", { className: "text-invalid" }, "Must have to select participant"), /* @__PURE__ */ React86.createElement("select", { value, onChange: handleChangeValue }, /* @__PURE__ */ React86.createElement("option", { value: "" }, "Select meeting host"), remoteParticipants.map((participant) => /* @__PURE__ */ React86.createElement("option", { value: participant.identity, key: participant.identity }, participant == null ? void 0 : participant.name))), /* @__PURE__ */ React86.createElement("div", { className: "button-container" }, /* @__PURE__ */ React86.createElement("button", { className: "lk-button tl-cancel", onClick: handleCancel }, "Cancel"), /* @__PURE__ */ React86.createElement("button", { className: "lk-button tl-ok", onClick: handleChange }, "Ok"))),
    /* @__PURE__ */ React86.createElement("div", { className: "arrow" }, /* @__PURE__ */ React86.createElement("div", { className: "arrow-shape" }))
  ));
}
...
function useToggleUserLink({ props }) {
  const { dispatch, state } = useLayoutContext().widget;
  const { className } = React87.useMemo(() => (0, import_components_core40.setupUserToggle)(), []);
  const mergedProps = React87.useMemo(
    () => mergeProps2(props, {
      className,
      onClick: () => {
        if (dispatch)
...
function UserToggle(props) {
  ...
  return /* @__PURE__ */ React87.createElement("button", __spreadValues({}, mergedProps), props.children);
}
...
function ControlBar(_a) {
  ...
  var _a2, _b2, _c, _d, _e, _f, _g;
  const layoutContext = useMaybeLayoutContext();
  const [isChatOpen, setIsChatOpen] = React90.useState(false);
  const [isShareLinkOpen, setIsShareLinkOpen] = React90.useState(false);
  const [isUserOpen, setIsUserOpen] = React90.useState(false);
  ...
  const showIcon = React90.useMemo(
    () => variation === "minimal" || variation === "verbose",
    [variation]
  );
  const showText = React90.useMemo(
    () => variation === "textOnly" || variation === "verbose",
    [variation]
  );
  const browserSupportsScreenSharing = (0, import_components_core41.supportsScreenSharing)();
  const [isScreenShareEnabled, setIsScreenShareEnabled] = React90.useState(false);
  ...
  return /* @__PURE__ */ React90.createElement("div", __spreadValues({}, htmlProps), visibleControls.microphone && /* @__PURE__ */ React90.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React90.createElement(TrackToggle, { source: import_livekit_client16.Track.Source.Microphone, showIcon }, showText && "Microphone"), /* @__PURE__ */ React90.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React90.createElement(MediaDeviceMenu, { kind: "audioinput" }))), visibleControls.camera && /* @__PURE__ */ React90.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React90.createElement(TrackToggle, { source: import_livekit_client16.Track.Source.Camera, showIcon }, showText && "Camera"), /* @__PURE__ */ React90.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React90.createElement(MediaDeviceMenu, { kind: "videoinput" }))), visibleControls.screenShare && browserSupportsScreenSharing && /* @__PURE__ */ React90.createElement(
    TrackToggle,
    {
  ...
  ), visibleControls.chat && /* @__PURE__ */ React90.createElement(ChatToggle, null, showIcon && /* @__PURE__ */ React90.createElement(ChatIcon_default, null), showText && "Chat", state && state.unreadMessages !== 0 && /* @__PURE__ */ React90.createElement("span", { className: "waiting-count" }, state.unreadMessages < 10 ? state.unreadMessages.toFixed(0) : "9+")), visibleControls.sharelink && /* @__PURE__ */ React90.createElement(ShareLinkToggle, null, showIcon && /* @__PURE__ */ React90.createElement(InviteIcon_default, null), showText && "Invite"), visibleControls.users && /* @__PURE__ */ React90.createElement(UserToggle, null, showIcon && /* @__PURE__ */ React90.createElement(UsersIcon_default, null), showText && "Participants", waitingRoomCount !== 0 && /* @__PURE__ */ React90.createElement("span", { className: "waiting-count" }, waitingRoomCount)), visibleControls.endForAll ? /* @__PURE__ */ React90.createElement("div", { className: "tl-leave lk-button-group" }, /* @__PURE__ */ React90.createElement("button", { className: "lk-disconnect-button" }, showIcon && /* @__PURE__ */ React90.createElement(LeaveIcon_default, null), showText && "Leave"), /* @__PURE__ */ React90.createElement("div", { className: "tl-leave-btn lk-button-group-menu" }, /* @__PURE__ */ React90.createElement(
    HostEndMeetingMenu,
    {
  ...
  ))) : /* @__PURE__ */ React90.createElement(DisconnectButton, null, showIcon && /* @__PURE__ */ React90.createElement(LeaveIcon_default, null), showText && visibleControls.leaveButton), /* @__PURE__ */ React90.createElement(StartAudio, { label: "Start Audio" }));
}
...
var ParticipantList = (_a) => {
  ...
  return /* @__PURE__ */ React91.createElement("div", __spreadValues({ style: { position: "relative" } }, elementProps), /* @__PURE__ */ React91.createElement(ParticipantContextIfNeeded, { participant: p }, children != null ? children : /* @__PURE__ */ React91.createElement(React91.Fragment, null, /* @__PURE__ */ React91.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React91.createElement("div", { className: "lk-participant-metadata-item" }, /* @__PURE__ */ React91.createElement(ParticipantName, null)), /* @__PURE__ */ React91.createElement("div", { className: "display-flex" }, /* @__PURE__ */ React91.createElement(
    TrackMutedIndicator,
    {
      source: import_livekit_client17.Track.Source.Microphone,
      show: "always"
...
var SvgApproveIcon = (props) => /* @__PURE__ */ React93.createElement(
  "svg",
  __spreadValues({
    width: "15",
    height: "11",
...
var SvgRejectIcon = (props) => /* @__PURE__ */ React94.createElement(
  "svg",
  __spreadValues({
    width: "15",
    height: "11",
...
function Users(_a) {
  ...
  const participants = useParticipants();
  const [waitingRoom, setWaitingRoom] = React95.useState([]);
  const [toggleWaiting, setToggleWaiting] = React95.useState(true);
  const room = useRoomContext();
  ...
  return /* @__PURE__ */ React95.createElement("div", __spreadProps(__spreadValues({}, props), { className: "lk-chat lk-users" }), /* @__PURE__ */ React95.createElement("div", { className: "lk-waitinroom" }, /* @__PURE__ */ React95.createElement("div", { className: "tl-waitingroom-heading" }, /* @__PURE__ */ React95.createElement("h3", null, "Waiting Room"), /* @__PURE__ */ React95.createElement("div", { className: "tl-toggle-switch" }, /* @__PURE__ */ React95.createElement(
    ToggleSwitch,
    {
      id: "toggleSwitch",
  ...
  )), toggleWaiting && waitingRoom.length ? /* @__PURE__ */ React95.createElement(
    "button",
    {
    ...
    "Approve All"
  ) : ""), waitingRoom.map((item) => /* @__PURE__ */ React95.createElement("div", { className: "tl-participant-li", key: item.username }, /* @__PURE__ */ React95.createElement("div", { className: "lk-participant-metadata" }, /* @__PURE__ */ React95.createElement("div", { className: "lk-participant-metadata-item" }, item.username), /* @__PURE__ */ React95.createElement("div", { className: "display-flex" }, /* @__PURE__ */ React95.createElement(
    "button",
    {
      className: "lk-button lk-waiting-room lk-success",
      onClick: () => admitUser(item.identity, "accepted")
    },
    /* @__PURE__ */ React95.createElement(ApproveIcon_default, null)
    ...
    /* @__PURE__ */ React95.createElement(RejectIcon_default, null)
  )))))), /* @__PURE__ */ React95.createElement("div", { className: "lk-participants" }, /* @__PURE__ */ React95.createElement("div", { className: "tl-participants-heading" }, /* @__PURE__ */ React95.createElement("h3", null, "Participants  ", /* @__PURE__ */ React95.createElement("span", null, "(", participants.length, ")"))), (participants == null ? void 0 : participants.length) ? /* @__PURE__ */ React95.createElement(ParticipantLoop, { participants }, /* @__PURE__ */ React95.createElement(ParticipantList, null)) : /* @__PURE__ */ React95.createElement("div", null, /* @__PURE__ */ React95.createElement("h5", null, "No Participants"))));
}
...
function VideoConference(_a) {
  ...
  const lastAutoFocusedScreenShareTrack = React96.useRef(null);
  const { localParticipant } = useLocalParticipant();
  const p = useEnsureParticipant(localParticipant);
  const { infoObserver } = React96.useMemo(() => {
  ...
  const [showShareButton, setShowShareButton] = React96.useState(false);
  const [showParticipantButton, setShowParticipantButton] = React96.useState(false);
  const [leaveButton, setLeaveButton] = React96.useState("Leave");
  const [endForAll, setEndForAll] = React96.useState(false);
  const meta = metadata ? JSON.parse(metadata) : {};
  const [waitingRoomCount, setWaitingRoomCount] = React96.useState(0);
  ...
  return /* @__PURE__ */ React96.createElement("div", __spreadValues({ className: "lk-video-conference" }, props), (0, import_components_core42.isWeb)() && /* @__PURE__ */ React96.createElement(
    LayoutContextProvider,
    {
      value: layoutContext,
      onWidgetChange: widgetUpdate
    },
    /* @__PURE__ */ React96.createElement("div", { className: "lk-video-conference-inner" }, !focusTrack ? /* @__PURE__ */ React96.createElement("div", { className: "lk-grid-layout-wrapper" }, /* @__PURE__ */ React96.createElement(GridLayout, { tracks }, /* @__PURE__ */ React96.createElement(ParticipantTile, null))) : /* @__PURE__ */ React96.createElement("div", { className: "lk-focus-layout-wrapper" }, /* @__PURE__ */ React96.createElement(FocusLayoutContainer, null, /* @__PURE__ */ React96.createElement(CarouselLayout, { tracks: carouselTracks }, /* @__PURE__ */ React96.createElement(ParticipantTile, null)), focusTrack && /* @__PURE__ */ React96.createElement(FocusLayout, { trackRef: focusTrack }))), /* @__PURE__ */ React96.createElement(
      ControlBar,
      {
        controls: {
          chat: true,
    ...
    showShareButton ? /* @__PURE__ */ React96.createElement(ShareLink, { style: {
      display: widgetState.showChat == "show_invite" ? "flex" : "none"
    } }) : /* @__PURE__ */ React96.createElement(React96.Fragment, null),
    showParticipantButton ? /* @__PURE__ */ React96.createElement(
      Users,
      {
  ...
  ), /* @__PURE__ */ React96.createElement(RoomAudioRenderer, null), /* @__PURE__ */ React96.createElement(ConnectionStateToast, null));
}
...
function AudioConference(_a) {
  ...
  const participants = useParticipants();
  return /* @__PURE__ */ React97.createElement(LayoutContextProvider, { onWidgetChange: setWidgetState }, /* @__PURE__ */ React97.createElement("div", __spreadValues({ className: "lk-audio-conference" }, props), /* @__PURE__ */ React97.createElement("div", { className: "lk-audio-conference-stage" }, /* @__PURE__ */ React97.createElement(ParticipantLoop, { participants }, /* @__PURE__ */ React97.createElement(ParticipantAudioTile, null))), /* @__PURE__ */ React97.createElement(
    ControlBar,
    {
      controls: { microphone: true, screenShare: false, camera: false, chat: true },
      waitingRoomCount: 0
    }
  ), widgetState.showChat == "show_chat" && /* @__PURE__ */ React97.createElement(Chat, null)));
}
