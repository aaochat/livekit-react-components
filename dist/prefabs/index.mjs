...
function useMediaTrackBySourceOrName(observerOptions, options = {}) {
  const [publication, setPublication] = React22.useState(getTrackByIdentifier(observerOptions));
...
function useChatToggle({ props }) {
  ...
  const { className } = React30.useMemo(() => setupChatToggle(), []);
...
function useToggleUserLink({ props }) {
  const { dispatch } = useLayoutContext().widget;
  const { className } = React63.useMemo(() => setupUserToggle(), []);
...
function VideoConference(_a) {
  ...
  return /* @__PURE__ */ React90.createElement("div", __spreadValues({ className: "lk-video-conference" }, props), isWeb() && /* @__PURE__ */ React90.createElement(
